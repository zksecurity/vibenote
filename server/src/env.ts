...
export function getEnv(): Env {
  ...
  const allowed = (process.env.ALLOWED_ORIGINS ?? 'http://localhost:3000')
    ...
    .map((s) => s.trim())
    .filter(Boolean);
  const viewerOrigins = (process.env.PUBLIC_VIEWER_ORIGINS ?? 'http://localhost:5173')
    .split(',')
    .map((s) => s.trim())
    .filter(Boolean);
  ...
  const env: Env = {
    ...
    PUBLIC_VIEWER_ORIGINS: viewerOrigins,
    PUBLIC_VIEWER_BASE_URL: must('PUBLIC_VIEWER_BASE_URL'),
    ...
    GIST_SERVICE_TOKEN: must('GIST_SERVICE_TOKEN'),
    SESSION_JWT_SECRET,
    SESSION_STORE_FILE,
    SHARE_STORE_FILE,
